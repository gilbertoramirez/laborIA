...
const modules = [
  ...
  { id: "04", label: "Casos", href: "/casos", icon: Briefcase },
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-border flex-col z-50">
      ...
      {/* Mobile bottom nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-border z-50 px-1 pb-[env(safe-area-inset-bottom)]">
          ...
          {modules.map((mod) => {
            ...
            return (
              <Link
                key={mod.id}
                href={mod.href}
                className={`flex flex-col items-center gap-0.5 py-2 px-1.5 min-w-0 transition-colors ${
                  isActive
                    ? "text-brand"
                    : "text-text-muted"
                }`}
              >
                ...
                <span className="text-[10px] truncate max-w-[56px]">{mod.label}</span>
              </Link>
            );
          })}
        </div>
